...
class Encoder(nn.Module):
    ...
    def __init__(
            self,
            num_types,user_num, d_model, d_inner,
        ...
        self.user_emb = nn.Embedding(user_num+1, d_model, padding_idx=Constants.PAD)  # dding 0
...
class Transformer(nn.Module):
    ...
    def __init__(
            self,
            num_types,user_num, d_model=256, d_rnn=128, d_inner=1024, n_layers=4, n_head=4, d_k=64, d_v=64, dropout=0.1,
            batch_size=32, device=0, ita=0.05, n_dis=4):
        super().__init__()

        self.encoder = Encoder(
            num_types=num_types, user_num=user_num, d_model=d_model, d_inner=d_inner,
