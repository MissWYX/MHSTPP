...
class LogisticMF():
    ...
    def predict(self, uid, lid):
        return self.user_vectors[uid].dot(self.item_vectors[lid])

    # This is the final prediction (by Hossein A. Rahmani)
    def predict_logistic(self, uid, lid):
        A = (self.user_vectors[uid].dot(self.item_vectors[lid])) + self.user_biases[uid] + self.item_biases[lid]
        eA = np.exp(A)
        return eA / (1 + eA)
