...
def to_npy(x):
    return x.cpu().data.numpy() if device == torch.device("cuda:2")  else x.detach().numpy()
...
class SelfAttn(nn.Module):
    ...
    def forward(self, joint, delta, traj_len):
        ...
        # #====
        # 如果 attn 是一个三维的张量，形状为 (batch_size, sequence_length, num_classes)，
        # 那么 F.softmax(attn, dim=-1) 将会对 num_classes 这个维度上的值进行 softmax，使得每个类别的分数在 0 到 1 之间，
        # 并且所有类别的分数之和为 1。而 batch_size 和 sequence_length 这两个维度将保持不变。
        # #
...
class Embed(nn.Module):
    ...
    def forward(self, traj_loc, mat2, vec, traj_len):
        ...
        for i in range(mask.shape[0]):  # N
            ...
            delta_s[i, :traj_len[i]] = torch.index_select(mat2, 0, traj_loc[i][:traj_len[i]])
...
class MultiEmbed(nn.Module):
    ...
    def forward(self, traj, mat, traj_len):
        ...
        for i in range(mask.shape[0]): # batch:N
            mask[i, 0:traj_len[i], 0:traj_len[i]] = 1
