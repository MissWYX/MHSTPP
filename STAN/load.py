...
def rst_mat1(traj, poi):
    ...
    for i, item in enumerate(traj):
        for j, term in enumerate(traj):
            poi_item, poi_term = poi[int(item[1]) - 1], poi[int(term[1]) - 1]  # retrieve poi by loc_id
            mat[i, j, 0] = haversine(lon1=poi_item[2], lat1=poi_item[1], lon2=poi_term[2], lat2=poi_term[1])
            mat[i, j, 1] = abs(float(item[2]) - float(term[2]))
...
def rt_mat2t(traj_time):  # traj_time (*M+1) triangle matrix
    ...
    for i, item in enumerate(traj_time):  # label
        ...
        for j, term in enumerate(traj_time[:i]):  # data
            mat[i-1, j] = np.abs(float(item) - float(term))
    return mat  # (*M, *M)
...
def process_traj(dname):  # start from 1
    ...
    dname = 'NYC_9000'
    data_path = '/data/wanghe/wyx/STAN/foursquare/NYC/stan_tr_top9000.lst'
    ...
    data = np.array(data)
    poi_path = '/data/wanghe/wyx/STAN/gowalla/poi_top_30000.csv'
    data_frame = pd.read_csv(poi_path, header=None)
    # data_frame.iloc[:, 1] = data_frame.iloc[:, 1] + 1
    
    # 将数据转换为数组
    poi = data_frame.iloc[:, [1,2,3]].to_numpy()
    
    num_user = data[-1, 0]  # max id of users, i.e. NUM
    data_user = data[:, 0]  # user_id sequence in data
    trajs, labels, mat1, mat2t, lens = [], [], [], [], []
    u_max, l_max = np.max(data[:, 0].astype(int)+1), np.max(data[:, 1].astype(int)+1)

    for u_id in set(data_user):
