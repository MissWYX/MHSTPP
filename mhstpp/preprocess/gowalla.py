...
def generate_data(user_count, top_n_item, min_length, max_length, data, BASE_DIR, DATA_SOURCE, small_data=False):
    ...
    # stan baseline :stan模型要多加一步，算出poi列表
    ...
    # # HGARN baseline
    # HGARN_item_poi = os.path.join(BASE_DIR, DATA_SOURCE, 'EEDN_poi_top{}.lst'.format(top_n_item))
    ...
    if os.path.exists(index2item_path) and os.path.exists(item2index_path):
        ...
        ##### ======== stan 模型需要poi地点信息
        poi = data[data['item_id'].isin(item_index2item)].iloc[:,[4, 2, 3]].drop_duplicates()
    ...
    for user_id, length in user_group.size().sort_values().iteritems():
        ...
        ##### ======== stan 模型需要用户会去的两个地点最远的距离，最大的时间差
        # 取出用户访问地点相差最大的位置，和时间差最大的值
        #####
